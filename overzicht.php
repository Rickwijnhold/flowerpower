...
<table class="container">
    ...
    $res=mysqli_query($conn, "SELECT artikel_name, Aantal, artikel_price FROM cart join cartartikels on cart.cart_id = cartartikels.cart_id where cart.cart_id = $id");
    $ress=mysqli_query($conn, "SELECT * FROM cart join cartartikels on cart.cart_id = cartartikels.cart_id where cart.cart_id = $id");
    $resss=mysqli_query($conn, "SELECT * FROM cart join cartartikels on cart.cart_id = cartartikels.cart_id join klant where klant.idKlant = $id");
    $totaal =0;
    ...
    {
        echo'Klant / Afleveradres <br>';echo $row['voornaamKlant']; echo '<br>';
        echo ''; echo $row['AdresKlant']; echo '<br>';
        echo 'postcode: '; echo $row['PostcodeKlant']; echo '<br><br>';

    }
    echo'<h6>Producten</h6>';
    while($row=mysqli_fetch_assoc($res))
    {
        $totaal=$totaal+$row['Aantal']*$row['artikel_price'];



        echo"<tr>";
        echo"<td>";echo $row['Aantal']; echo "</td>";
        echo"<td>"; echo $row['artikel_name']; echo "</td>";
        echo"<td>"; echo $row['artikel_price'];  echo "</td>";
        echo"</tr>";
